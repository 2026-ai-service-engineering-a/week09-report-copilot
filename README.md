...
이 저장소는 그 둘을 **한 벌로** 만든다. 화면이 그리는 리포트 문서와 에이전트가
읽고 쓰는 상태가 같은 객체이고, 요청이 그 문서와 "무엇을 골랐는지"를 함께
싣는다.
...
POST /agent
  ...
  forwardedProps
    selectedSectionId  무엇을 고른 채로 말했는지   ← "이거"가 무엇인지가 여기 있다
    uiAction           버튼·드롭다운으로 조작한 것
```

화면은 **완성된 제품 하나**다. 모드를 고르는 스위치가 없다. 10주차부터 각자
만들 것도 이 모양이다.

대신 API에는 비교 대상이 남아 있다. `POST /report`는 폼형이고 `POST /chat`은
화면을 보지 못하는 챗 위젯이다. 셋을 나란히 돌려 무엇이 갈리는지 보는 것은
예제의 일이다.
...
```plaintext
① v1 · 챗 위젯 (POST /chat)     요청에 실린 선택: 없음
   답: 무엇을 바꿀지 알기 어렵습니다. 섹션을 고르고 다시 말씀해 주세요.

② v2 · 공유 상태 (POST /agent)  요청에 실린 선택: s2
   패치: [{'op': 'remove', 'path': '/sections/1'}]
```

**같은 모델, 같은 그래프, 같은 한마디다.** 갈린 것은 요청의 모양 하나이고
코드로는 `selected=None` 한 줄이다.
